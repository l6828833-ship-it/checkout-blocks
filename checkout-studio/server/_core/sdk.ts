...
import { establishShopifyEmbeddedSession, ShopifyEmbeddedAuthError } from "../shopifyEmbedded";
import { ENV } from "./env";
import type {
...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    // existing Manus preview-session fallback. A failed non-Shopify bearer
    // token falls through so local preview authentication continues to work.
    if (bearerToken && ENV.shopifyApiKey && ENV.shopifyApiSecret) {
      try {
        const shopifySession = await establishShopifyEmbeddedSession(bearerToken);
        return shopifySession.user;
      } catch (error) {
