...
describe.skipIf(!configured)("shopify smoke (live)", () => {
  it(
    "accepts an empty catalog or verifies usable product data when products exist",
    { timeout: 30_000 },
    async () => {
    ...
    // Checkout Studio is an embedded configuration workspace rather than a
    // storefront. A newly-created connected store may legitimately have no
    // catalog yet, so only validate product completeness when items exist.
