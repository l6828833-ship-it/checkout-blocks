...
import { describeLiveApplyBlock } from "../../../shared/studioSafety";
...
export function StudioOverlays() {
  ...
  const connectionState = (workspaceQuery.data?.connection.state ?? "not_connected") as "not_connected" | "checking" | "ready" | "error" | "denied";
  const liveApplyBlock = describeLiveApplyBlock({
    connectionState,
    capabilityMessage: workspaceQuery.data?.connection.message,
  });
  const canPublish = connectionState === "ready";
